...
class ContractedPauliDecomposition(PauliDecomposition):
    ...
    def contract_pauli_terms(self,
        ...
        for i1 in range(nstrings):
            for i2 in range(i1+1,nstrings):
                ...
                self.contraction_map.append([(pauli_string_1, pauli_string_2),
                ...
                if contracted_pauli_string not in self.unique_pauli_strings:
                    ...
                    self.contracted_circuits.append(self._create_circuit(contracted_pauli_string))
                ...
                # otherwise find reference of existing circuit
                    ...
                    self.contraction_index_mapping.append(self.unique_pauli_strings.index(contracted_pauli_string))
    ...
    def group_contracted_terms(self):
        ...
        def get_eigenvalues(pauli_string):
            ...
            """
            ev_dict = {
                'X':[1,-1], 'Y':[1,-1], 'Z':[1,-1], 'I':[1,1]
            }
            evs = ev_dict[pauli_string[-1]]
            for p in pauli_string[:-1][::-1]:
        ...
        def string_qw_commutator(pauli1, pauli2):
            ...
            return np.all([ (p1==p2) | (p1=='I') | (p2=='I') for p1,p2 in zip(pauli1, pauli2)])
        ...
        for i1 in range(nstrings):
            for i2 in range(i1+1, nstrings):
                pauli_string_1, pauli_string_2 = self.unique_pauli_strings[i1], self.unique_pauli_strings[i2]
        ...
        # greedy clustering 
        qwc_groups_flat = nx.coloring.greedy_color(qwc_complement_graph, strategy='largest_first')

        # organize the groups
        qwc_groups = OrderedDict()
        qwc_groups_eigenvalues = OrderedDict()
        for pauli, group_id in qwc_groups_flat.items():
            if group_id not in qwc_groups:
                qwc_groups[group_id] = []
                qwc_groups_eigenvalues[group_id] = []
            qwc_groups[group_id].append(pauli)
            qwc_groups_eigenvalues[group_id].append(get_eigenvalues(pauli))

        # determine the shared eigenbasis
        qwc_groups_shared_basis_transformation = OrderedDict()
        for group_id, pauli_strings in qwc_groups.items():
            ...
            qwc_groups_shared_basis_transformation[group_id] = create_shared_basis_circuit(shared_basis)

        
        return qwc_groups, qwc_groups_eigenvalues, qwc_groups_shared_basis_transformation
